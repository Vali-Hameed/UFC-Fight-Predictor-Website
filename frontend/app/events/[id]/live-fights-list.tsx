...
function CollapsibleCanceledFights({ fights, isEventStarted, isArchived }: { fights: FightCardData[], isEventStarted: boolean, isArchived: boolean }) {
  const [isOpen, setIsOpen] = useState(false);

  if (fights.length === 0) return null;

  return (
    <div className="mt-8 rounded-3xl border border-white/10 bg-white/5 p-5 shadow-sm">
      <button 
...
export function LiveFightsList({ eventId, initialFights, isEventStarted, isArchived }: LiveFightsListProps) {
  ...
  const activeFights = fights.filter(f => f.fight.status !== "CANCELED");
  const canceledFights = fights.filter(f => f.fight.status === "CANCELED");

  return (
    <div className="space-y-4">
      {activeFights.map(({ fight, mlPrediction, communityVote }) => (
        <PredictionCard 
          key={fight.id} 
          fight={fight} 
          mlPrediction={mlPrediction} 
      ...
      <CollapsibleCanceledFights 
        fights={canceledFights} 
        isEventStarted={isEventStarted} 
        isArchived={isArchived} 
      />
    </div>
